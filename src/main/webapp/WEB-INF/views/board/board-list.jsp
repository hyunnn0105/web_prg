...
<body>
            ...
            <div class="bottom-section">
                ...
                <nav aria-label="Page navigation example">
                    <ul class="pagination pagination-lg pagination-custom">
                        <!-- ** 여기 prev&next 링크 수정하기 (220802 수정완료) -->
                        <c:if test="${pm.prev}">
                            <li class="page-item"><a class="page-link" href="/board/list?pageNum=${pm.beginPage - 1}&amount=10&type=${s.type}&keyword=${s.keyword}">prev</a></li>
                        </c:if>
                      ...
                      <c:forEach var="n" begin="${pm.beginPage}" end="${pm.endPage}" step="1">
                        ...
                        <c:if test="${pm.next}">
                            <li class="page-item"><a class="page-link" href="/board/list?pageNum=${pm.endPage + 1}&amount=10&type=${s.type}&keyword=${s.keyword}">next</a></li>
                        </c:if>
                    </ul>
                </nav>
    ...
    <script>
        ...
        let get = document.getElementById('pageNum').innerText;
        ...
        if(current){
                for (let i = 0; i < get.length; i++) {
                    get[i].addEventListener("click", function name(params) {
                        let btn = e.target.parentElement.firstElementChild.textContent
                        console.log('btn : ', btn);
                    }
                    )
        ...
        */
        ...
        function detailEvent() {
            ...
            // <li data-page-num="${n}" class="page-item" id="pageNum">
                ...
                // /board/list?pageNum=${n}&amount=${pm.page.amount}

                /*
            // 페이지 정보 알려주기 -> 나갈때 다시 그 자리로
            let pagedata = $li.dataset.pageNum;
            console.log('pd :', pagedata);
            let pageamout = '{pm.page.amount}';
            console.log('pa', pageamount);
                */
