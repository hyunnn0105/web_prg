...
import static com.project.web_prg.util.LoginUtils.getCurrnetMemberAuth;
...
//            dispatcher.forward(request, response);
    ...
    public void postHandle(HttpServletRequest request, HttpServletResponse response, Object handler, ModelAndView modelAndView) throws Exception {
        ...
        if (uriList.contains(requestUri) && method.equalsIgnoreCase("GET")) {
            ...
            log.info("login account - {}", getCurrnetUtil(session));
    ...
    private boolean isAdmin(HttpSession session) {
        return getCurrnetMemberAuth(session).equals("ADMIN");
    }

    // 게시글 정보 == 세션정보 -> 나다.
    private static boolean isMine(HttpSession session, String account) {
        return account.equals(getCurrnetUtil(session));
    }
